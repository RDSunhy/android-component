...
import top.sunhy.image.progress.GlideImageViewTarget
import top.sunhy.image.progress.ProgressManager
import top.sunhy.image.transformation.*
import top.sunhy.ktx.getAppScopeContext
import java.io.File
import java.net.URL
...
object GlideImageLoader {
    ...
    private var maxMemory = 1 * 1024 * 1024
    ...
    private fun glideRequests(context: Any?): GlideRequests {
        return when (context) {
            is Context -> ShyGlideModule.with(context)
            is Activity -> {
                if (context.isDestroyed){
                    ShyGlideModule.with(getAppScopeContext())
                }else{
                    ShyGlideModule.with(context)
                }
            }
            is FragmentActivity -> {
                if (context.isDestroyed){
            ...
            is Fragment -> ShyGlideModule.with(context)
            is android.app.Fragment -> ShyGlideModule.with(context)
            is View -> ShyGlideModule.with(context)
