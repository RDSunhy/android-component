...
import top.sunhy.image.core.ImageOptions.DrawableOptions.Companion.DEFAULT
import top.sunhy.image.core.OnImageListener
import top.sunhy.image.progress.OnProgressListener
...
fun ImageView.load(
    ...
    this.load(load = drawableId, placeHolderResId = drawableId, errorResId = errorId)
}

@JvmOverloads
fun ImageView.loadWithRes(
    url: String?,
    placeHolder: Drawable? = DEFAULT.placeHolderDrawable,
    errorId: Drawable? = DEFAULT.errorDrawable,
    requestListener: (OnImageListener.() -> Unit)? = null
) {
    this.load(load = url, placeHolderDrawable = placeHolder, errorDrawable = errorId,
...
fun ImageView.load(
    ...
    this.load(load = url, placeHolderResId = placeHolder, errorResId = errorId,
        requestListener = requestListener)
}
...
fun ImageView.loadProgress(
    ...
    this.load(load = url, placeHolderResId = placeHolder, errorResId = placeHolder,
        onProgressListener = progressListener)
}

@JvmOverloads
fun ImageView.loadResize(
    url: String?,
    width: Int,
    height: Int,
    @RawRes @DrawableRes placeHolder: Int = DEFAULT.placeHolderResId
...
fun ImageView.loadRoundCorner(
    ...
    this.load(load = url, placeHolderResId = placeHolder, errorResId = placeHolder,
        isRoundedCorners = radius > 0, roundRadius = radius, cornerType = type, centerCrop = true)
}
...
fun ImageView.loadCircle(
    ...
    this.load(load = drawableId, placeHolderResId = placeHolder, errorResId = placeHolder,
