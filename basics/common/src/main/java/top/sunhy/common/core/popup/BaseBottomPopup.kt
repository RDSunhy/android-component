...
abstract class BaseBottomPopup<B : ViewDataBinding, VM : BaseViewModel>(activity: AppCompatActivity) :
    BottomPopupView(activity), IBasePopup {
    ...
    override fun initPopupContent() {
        ...
        popupInfo.isDestroyOnDismiss = true //防止内存泄漏
        popupInfo.isMoveUpToKeyboard = false //键盘默认不把弹窗顶上去 子类可以修改
        popupInfo.popupAnimation = PopupAnimation.TranslateFromBottom
        mBinding = DataBindingUtil.bind(popupImplView)
            ?: throw NullPointerException("XmxBottomPopup mBinding is Null!")
