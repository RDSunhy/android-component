...
fun FragmentActivity.loadFragments(
    ...
    loadFragmentsTransaction(containerViewId, showPosition, supportFragmentManager, *fragments,isAnimation = isAnimation)
}
 ...
 * @param showFragment 需要显示的fragment
 */
fun FragmentActivity.showHideFragment(showFragment: Fragment,isAnimation: Boolean = false) {
    showHideFragmentTransaction(supportFragmentManager, showFragment,isAnimation = isAnimation)
}
 ...
 * @param showFragment
 */
private fun showHideFragmentTransaction(fragmentManager: FragmentManager, showFragment: Fragment, isAnimation: Boolean = false) {
    val beginTransaction = fragmentManager.beginTransaction()
    if(isAnimation){
        beginTransaction.setTransition(FragmentTransaction.TRANSIT_FRAGMENT_FADE)
    }
